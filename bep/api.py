...
@decorate_view(cache_page(settings.CACHE_SECONDS))
def towns(request):
    return Town.objects.select_related('county', 'county__nation').order_by('label').all()

@api.get("/provinces", response=PaginatedResponseSchema[ProvinceSchema], url_name='provinces')
@paginate
@decorate_view(cache_page(settings.CACHE_SECONDS))
def provinces(request):
    return Province.objects.prefetch_related('nation').order_by('label').all()

@api.get("/dioceses", response=PaginatedResponseSchema[DioceseSchema], url_name='dioceses')
@paginate
@decorate_view(cache_page(settings.CACHE_SECONDS))
def dioceses(request):
    return Diocese.objects.select_related('province', 'province__nation').order_by('label').all()
...
def archdeaconries(request):
    return Archdeaconry.objects.select_related('diocese', 'diocese__province', 'diocese__province__nation').order_by('label').all()
